...
def generateStatisticalAnalysis(predecessor):
    for distribution in predecessor:
        if distribution is None:  
            yield None
            break
        distribution_type, param1, param2, count, data = distribution
        mean = round(np.mean(data), 0)
        median = round(np.median(data), 0)
        try:
            mode = round(stats.mode(distribution[4]), 6)
        except stats.StatisticsError:
            mode = None
        pstdev = round(stats.pstdev(data), 0)
        min_num = round(np.min(data), 0)
        max_num = round(np.max(data), 0)
        yield (distribution_type, param1, param2, count, mean, median, mode, pstdev, min_num, max_num)
        #print("Generated Result: ", result)                                                                                   #MAYBE
...
def saveStatisticalAnalysisCSV(predecessor, csvWriter, writeHeader):
    if writeHeader:
        csvWriter.writerow(['Distribution', 'Param1', 'Param2', 'Count', 'Mean', 'Median', 'Mode', 'Pstdev', 'Min', 'Max'])
