...
def genSamples(wavetype, samples, wavecol, table2D, dutyCycle,
    ...
    def genTri():
        '''
        Linear Triangle starting at lowest point -1, at the 
        halfway point rising to highest point 1, and then 
        decending to -1 at the final sample.
        '''
        halfSamples = math.floor(samples / 2)
        startRadians = -1
        midRadians = 2
        endRadians = -1
        stepRadians = round(midRadians / halfSamples,10)
        curRadians = startRadians
        for step in range(0, halfSamples):
        ...
        for step in range(halfSamples, samples):
            table2D[step][wavecol] = __scalePercentToWavRange__(
                curRadians)
            curRadians += stepRadians
    ...
    def genCosine():
        '''
        Cosine wave starting at highest point at time 1,
        falls to lowest point at time -1, and then 
        rises back to 1.
        '''
        startRadians = 0
        stepRadians = (-2.0 * math.pi) / (samples-1) # full 360 cut into pieces
    ...
    def genSquare():
        '''
        Linear half square that generates a line at the
        minimum value of -1 until the halfway point,
        where it creates the 2nd line at the maximum
        value of 1 until the end.
        '''
        halfSamples = math.floor(samples / 2)
    ...
    def genPulse():
        '''
        Linear pulse starts at the minimum value of 
        -1, rises immediately to the maximum value 
        of 1 when it reaches the loop counter index 
        of DutyCycle, and then stays at 1 until the end.
        '''
        startRatio = -1
        endRatio = 1
        for step in range(0, samples):
            if round(step/(samples-1),10) < (1.0 - dutyCycle): #may have been misedited?
                table2D[step][wavecol] = __scalePercentToWavRange__(
                    startRatio)
            else:
                table2D[step][wavecol] = __scalePercentToWavRange__(
    ...
    def genFallSaw():
        '''
        Falling sawtooth starting at the maximum 
        value of 1 and linearly and incrementally 
        falling to -1.
        '''
        stepsize = (-2.0) / (samples-1)   # distance / steps up from 0
        curvalue = 1.0
        for step in range(0, samples):
            intvalue = __scalePercentToWavRange__(curvalue)
            table2D[step][wavecol] = intvalue
